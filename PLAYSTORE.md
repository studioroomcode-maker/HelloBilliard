...
## 결제 버튼 숨기기 (구현 완료 — 설정만 하면 됨)

앱에 **스토어 빌드 감지**가 들어가 있다. 스토어 빌드로 인식되면 Pro 모달에서
**'Pro 구매하기'·'후원하기' 버튼이 자동으로 사라지고 언락 코드 입력만 남는다.**

인식되는 조건 (둘 중 하나면 됨):
1. **자동** — TWA로 열면 `document.referrer`가 `android-app://`로 시작해 자동 감지됨 ✓
2. **명시** — 시작 URL에 `?store=play`를 붙이면 확실히 적용되고 기기에 저장됨
   (PWABuilder 생성 프로젝트에서 launch URL을 `https://hellobilliard.studioroomkr.com/?store=play`로)

> 테스트: 브라우저 콘솔에서 `__hbSetStore('play')` → 구매 버튼 사라짐 / `__hbSetStore(null)` → 원복
